	...
	<div class="card contact-card">
		...
		<div class="list-group list-group-flush">
			...
			<div class="list-group-item contact-card-list-item">
				<router-link v-if="!venue.has_owner" to="/promote" class="float-right">modifica</router-link>
				...
				<p class="mb-0"><a :href="googleMapsUrl" target="_blank">Come arrivare</a></p>
			</div>
			...
			<div class="list-group-item contact-card-list-item">
				...
				<template v-if="venue.has_owner && venue.business_hours.length">
					<a href="#" :class="isOpen ? 'text-success' : 'text-danger'" @click.prevent="toggleHours">
						{{ isOpen ? 'Aperto ora' : 'Chiuso ora'}}<pg-icon :icon="hoursExpanded ? 'chevron-up' : 'chevron-down'" class="ml-1 contact-card-chevron-icon" />
					</a>
					<table v-if="hoursExpanded" v-cloak>
						<tr v-for="row in businessHoursRows">
							<td class="align-top pr-3">{{ row.day }}</td>
							<td>
								<div v-if="!row.hours.length" class="text-muted">Chiuso</div>
				...
				<p class="mb-0 text-muted" v-else>Nessun orario</p>
			</div>
			...
			<div class="list-group-item contact-card-list-item">
				...
				<p class="mb-0 text-muted" v-else>Nessuna informazione di contatto</p>
			</div>
			...
			<div class="list-group-item contact-card-list-item">
				...
				<ul class="list-unstyled mb-0" v-if="hasUrls">
					...
					<li v-if="venue.urls.facebook"><a :href="venue.urls.facebook" target="_blank">Pagina Facebook</a></li>
				</ul>
				<p class="mb-0 text-muted" v-else>Nessun sito o pagina social</p>
			</div>
		</div>
	</div>
</template>